...
import module.dendrite as dendrite
import module.dend_compartment as dend_compartment
import module.wiring as wiring
import module.soma as soma
...
class BaseDendNeuron(base.MemoryModule, abc.ABC):
    ...
    def reshape_input(
        ...
        if len(self.soma_shape) == 2:
            # after a nn.Linear layer
            x_transformed = x.reshape([*shape_prefix, *self.soma_shape, -1])
            #print(f"after transformation{x_transformed.shape}")
            #print(x.reshape([*shape_prefix, *self.soma_shape, -1]).shape)
            return x_transformed
        elif len(self.soma_shape) > 2:
            ...
            x_transformed = x.permute(perm_trans)
            #print(f"after transformation{x_transformed.shape}")
            return x_transformed
    ...
    def multi_step_forward(self, x_seq: torch.Tensor,hook) -> torch.Tensor:
        print("base")
        T = x_seq.shape[0]
        soma_spike_seq = []
    ...
    def forward(self, x: torch.Tensor,hook=None) -> torch.Tensor:
        if self.step_mode == "s":
            return self.single_step_forward(x)
        elif self.step_mode == "m":
            return self.multi_step_forward(x,hook=hook)
        else:
            raise ValueError(
                f"BaseDendNeuron.step_mode should be 'm' or 's', "
...
class VForwardDendNeuron(BaseDendNeuron, abc.ABC):
    ...
    def __init__(
        self, dend: dendrite.SegregatedDend, soma: soma.LIFSoma,
        soma_shape: List[int], 
        ...
        Returns:
            torch.Tensor: somatic input with shape [N, *soma_shape] or [T, N,
                *soma_shape]. If v_soma or v_dend_output has a prepending T
                ...
                dimension.
        """
        v_dend_all = v_dend_output.sum(-1)
        return v_dend_all*self.forward_strength
...
class DendriticLIF(nn.Module):  ###改这个函数
    def __init__(self, num_branches):
        super().__init__()
        # ... 其他初始化 ...
        
        # 1. 关键：为每个分支定义独立的 BatchNorm
        # num_features = num_branches, 这样每个分支有自己的 mean/var
        self.branch_bn = nn.BatchNorm1d(num_branches) 
        
        # 2. 每个分支独立的可学习门控参数
        self.gate_scale = nn.Parameter(torch.ones(1, num_branches, 1, 1))
        self.gate_bias = nn.Parameter(torch.zeros(1, num_branches, 1, 1) - 1.0) # 初始负偏置

    def forward(self, v_dend):
        # x shape: (Batch, Time, Branch, H, W) -> 需要调整维度以适应 BN
        
        # ... 积分得到 v_dend (Batch, Time, Branch, H, W) ...
        
        #处理 BN: 需要把 (B, T, Branch, H, W) 里的 Branch 放到 Channel 维，其他合并
        # 这里简化写，实际要 reshape
        B, T, N, H, W = v_dend.shape
        v_flat = v_dend.view(B * T, N, H * W) 
        # 注意：BatchNorm1d 通常处理 (N, C, L)，这里 C=num_branches
        
        # 应用归一化：把所有分支拉到同一水平
        v_norm = self.branch_bn(v_flat) 
        v_norm = v_norm.view(B, T, N, H, W)
        
        # 应用门控：此时可以用统一的逻辑，因为幅度已经对齐了
        gate = torch.sigmoid(self.gate_scale * v_norm + self.gate_bias)
        y = v_norm * gate
        
        # Sum to Soma
        i_soma = y.sum(dim=2) # Sum over branches
...
class VActivationForwardDendNeuron(VForwardDendNeuron):
    ...
    def multi_step_forward(self, x_seq: torch.Tensor,hook=None) -> torch.Tensor:
        ...
        """
        x_seq = self.reshape_input(x_seq, "m") # shape=[T, N, n_soma, n_input]
        #print(f"here shape is {x_seq.shape}")
        self.dend.step_mode = "m"
