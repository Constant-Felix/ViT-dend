...
class CIFAR10Net(nn.Module):
    def __init__(self, channels, class_num: int, T: int=32, P:int=-1,num_branches=8,compartments_per_branch=6,branch_degree=1):
        super().__init__()
        conv = []
        for i in range(2):
            for j in range(3):
...
def main():
    ...
    out_dir = f'{args.neu}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}_{datetime.now().strftime("%Y%m%d-%H%M%S")}'
    if args.amp:
        out_dir += '_amp'
