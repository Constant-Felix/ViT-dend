...
def main():
    ...
    if args.local_rank != -1 or env_world_size > 1:
        args.distributed = True

# default device for single-process run
    if not args.distributed:
        args.local_rank = 0
        args.rank = 0
        args.world_size = 1
        args.device = "cuda:0"  # do NOT hardcode other card like cuda:1
        _logger.info("Training with a single process on 1 GPU.")
    else:
    # For distributed run: bind device to LOCAL_RANK first, then init process group
    # torchrun normally sets LOCAL_RANK, RANK, WORLD_SIZE
        if args.local_rank == -1:
        # fallback: try computing from RANK and local GPU count (less reliable)
            args.local_rank = int(os.environ.get("RANK", 0)) % torch.cuda.device_count()

    # bind this process to the proper CUDA device
    ...
    _logger.info(
        "Distributed mode: global rank %d / %d, local_rank %d, device %s, CUDA_VISIBLE_DEVICES=%s"
        % (args.rank, args.world_size, args.local_rank, args.device, os.environ.get("CUDA_VISIBLE_DEVICES"))
    )
    ...
    if args.dend == False:
        ...
        model = create_model(
            ...
            dvs_mode=args.dvs_mode,
            TET=args.TET,
            num_compartment = args.num_compartment,
            dend = args.dend,
    ...
    try:
        ...
        if args.local_rank == 0:
            ...
            _logger.info("top-1:%.3f", eval_metrics["top1"])
...
def validate(
    ...
    #fr_dict, nz_dict = {"t0": dict(), "t1": dict(), "t2": dict(), "t3": dict(), "t4": dict(), "t5": dict(), "t6": dict(), "t7": dict(),"t8":dict(), "t9":dict()}, {
    #    "t0": dict(),
    #    "t1": dict(),
    #    "t2": dict(),
    #    "t3": dict(),
    #    "t4": dict(), "t5": dict(), "t6": dict(), "t7": dict(), "t8": dict(), "t9": dict()
    #}
    with torch.no_grad():
