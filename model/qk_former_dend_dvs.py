...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.,
        ...
        if dend==False:
            self.mlp1_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        self.mlp2_conv = nn.Conv2d(in_features//num_compartment, num_compartment*hidden_features, kernel_size=1, stride=1) if dend == True else nn.Conv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp2_bn = nn.BatchNorm2d(num_compartment*hidden_features) if dend == True else nn.BatchNorm2d(hidden_features)
        if dend==False:
            self.mlp2_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
...
class Token_dend_QK_Attention(nn.Module):   ###改成conv2d
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,dend=False,integer=False,
        ...
        self.bn_alter = bn_alter
        self.f_da = lambda x:x
        self.q_conv = nn.Conv2d(dim, dim*num_compartment, kernel_size=1, stride=1, bias=False) if dend == True else nn.Conv2d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.q_bn = nn.BatchNorm2d(dim*num_compartment) if dend == True else nn.BatchNorm2d(dim)

        if dend==False:
            self.q_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        self.k_conv = nn.Conv2d(dim, dim*num_compartment, kernel_size=1, stride=1, bias=False) if dend == True else nn.Conv2d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm2d(dim*num_compartment) if dend == True else nn.BatchNorm2d(dim)
        if dend==False:
            self.k_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        self.proj_conv = nn.Conv2d(dim, dim*num_compartment, kernel_size=1, stride=1, bias=False) if dend == True else nn.Conv2d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.proj_bn = nn.BatchNorm2d(dim*num_compartment) if dend == True else nn.BatchNorm2d(dim)
        if dend==False:
            self.proj_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
...
class Spiking_dend_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
        ...
        if dend==False:
            self.v_lif = MultiStepParametricLIFNode(decay_input=True, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
            self.dc3 = _make_dend_compartment(dim*num_compartment, num_compartment, multi=multi, soma_astro=soma_astro)
            self.wr3 = wiring.SegregatedDendWiring(num_compartment)
            self.dend3 = dendrite.SegregatedDend(step_mode='m',compartment=self.dc3,wiring=self.wr3)
...
class PatchEmbedInit(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256,sps_integer=False):
        ...
        if sps_integer==False:
            self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        if sps_integer==False:
            self.proj1_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        if sps_integer==False:
            self.proj2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        if sps_integer==False:
            self.proj3_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
        ...
        if sps_integer==False:
            self.proj_res_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
...
class PatchEmbeddingStage(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256,sps_integer=False):
        ...
        if sps_integer==False:
            self.proj4_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, surrogate_function=surrogate.ATan())
        else:
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer_dend(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios, dend=dend,integer=integer,num_compartment=num_compartment,multi=multi,bn_alter=bn_alter,soma_astro=soma_astro)  ##在这改了第一个block为点神经元
            for j in range(1)])
