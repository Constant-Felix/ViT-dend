...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.,
                 dend = False,integer = False,multi = False,num_compartment=2):
        super().__init__()
...
class Spiking_dend_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,
                 dend=False,integer=False,num_compartment=2,multi=False):
        ...
        if dend==False:
            ...
            if multi==False:
                ...
                self.dc4 = dend_compartment.MultiScaleDendCompartment(num_compartment,step_mode='m')    
...
class TokenSpikingTransformer_dend(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1,dend=False,integer=False,num_compartment=2,multi=False):
        super().__init__()
        self.tssa = Token_dend_QK_Attention(dim, num_heads,qkv_bias=qkv_bias,qk_scale=qk_scale,attn_drop=attn_drop,
                                               proj_drop=drop_path,sr_ratio=sr_ratio, dend=dend,integer=integer,num_compartment=num_compartment,multi=multi)
        mlp_in_features = int(dim/num_compartment)
        self.mlp = MLP(in_features= mlp_in_features, hidden_features=dim, drop=drop,dend=dend,integer=integer,num_compartment=num_compartment,multi=multi)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, pretrained_cfg=None, in_chans = 3, no_weight_decay = None, dend=False,integer=False,num_compartment=2,multi=False,sps_integer=False
                 ):
        super().__init__()
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer_dend(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios, dend=dend,integer=integer,num_compartment=num_compartment,multi=multi)
            for j in range(1)])
