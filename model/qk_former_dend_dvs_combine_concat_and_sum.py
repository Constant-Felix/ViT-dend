...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.,
        ...
        self.mlp1_lif = dendneuron(channels=hidden_features, soma_shape=np.zeros((3,),int),dend_model=dend, integer=integer, multi=multi, num_compartment=num_compartment, concat=False)    
        ...
        self.mlp2_lif = dendneuron(channels=out_features*num_compartment, soma_shape=np.zeros((3,),int),dend_model=dend, integer=integer, multi=multi, num_compartment=num_compartment, concat=False) 
...
class TokenSpikingTransformer_dend(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        mlp_in_features = int(dim * mlp_ratio * num_compartment)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_in_features, drop=drop,dend=dend,integer=integer,num_compartment=num_compartment,multi=multi,bn_alter=bn_alter)
...
@register_model
def QKFormer_dend_combine_dvs(pretrained=False, **kwargs):
    model = vit_snn(
        **kwargs
    )
