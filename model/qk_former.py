...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., psn=False):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.mlp1_conv = nn.Conv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp1_bn = nn.BatchNorm2d(hidden_features)
        self.dend1 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=hidden_features)
        self.mlp1_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)

        self.mlp2_conv = nn.Conv2d(hidden_features, out_features, kernel_size=1, stride=1)
        self.mlp2_bn = nn.BatchNorm2d(out_features)
        self.dend2 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=hidden_features)
        self.mlp2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)
...
class Token_QK_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,psn=False):
        super().__init__()
        ...
        self.dend1 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=dim)
        self.q_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)

        self.k_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)
        self.dend2 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=dim)
        self.k_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)

        self.attn_lif = MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16) ###

        self.proj_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1)  
        self.proj_bn = nn.BatchNorm1d(dim)
        self.dend3 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=dim)
        self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)
    ...
    def forward(self, x):
        ...
        q_conv_out = self.dend1(q_conv_out)
        ...
        k_conv_out = self.dend2(k_conv_out)
...
class Spiking_Self_Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,psn=False):
        ...
        self.dend3 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=dim)
        self.v_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)
        self.attn_lif = MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)

        self.proj_conv = nn.Conv1d(dim, dim, kernel_size=1, stride=1)
        self.proj_bn = nn.BatchNorm1d(dim)
        self.dend4 = dend_compartment.SparseChannelPreservingTrunkDistalDendCompartment(channels=dim)
        self.proj_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy') if psn==False else soma.MaskedSlidingPSN(order=16)
    ...
    def forward(self, x):
        ...
        v_conv_out = self.dend3(v_conv_out)
        ...
        x = self.attn_lif(x)
        x = x.flatten(0, 1)
        x = self.proj_lif(self.dend4(self.proj_bn(self.proj_conv(x)).reshape(T,B,C,N))).reshape(T, B, C, W, H)
        return x

class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1,psn=False):
        super().__init__()
        self.tssa = Token_QK_Attention(dim, num_heads,psn=psn)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop,psn=psn)
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1,psn=False):
        super().__init__()
        self.ssa = Spiking_Self_Attention(dim, num_heads,psn=psn)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop,psn=psn)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, pretrained_cfg=None, in_chans = 3, no_weight_decay = None,TET=False,psn=False
                 ):
        super().__init__()
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer(
            ...
            norm_layer=norm_layer, sr_ratio=sr_ratios,psn=psn)
            for j in range(1)])
...
def QKFormer(pretrained=False, **kwargs):
    model = vit_snn(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=4, sr_ratios=1, #psn=True,
        **kwargs
    )
